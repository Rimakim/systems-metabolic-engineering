...
if __name__=='__main__':
    ...
    class OptReg:
        ...
        def __init__(self, model_path, biomass_lb, target_rxn, L, C, eps, oxygen_condition, iter_num):
        ...
        def calculate_cellular_state(self):
            ...
            for rxn, flux in zip(self.model.reactions, sol.fluxes):
                self.vo[rxn.id] = flux
        ...
        def add_constraints(self):
            ...
            for var in self.v.keys():
                ...
                self.m.addConstr(self.v[var] >= ( self.var_o[var]*(1-self.C) + self.v_ub[var]*self.C ) * ( 1-self.yu[var] ) + ( self.v_lb[var] * self.yu[var] ))
                self.m.addConstr(self.v[var] <= self.v_ub[var])
                
                # 4. Down regulation constraints
                self.m.addConstr(self.v[var] >= self.v_lb[var])
                self.m.addConstr(self.v[var] <= ( self.var_o[var]*(1-self.C) + self.v_lb[var]*self.C ) * ( 1-self.yd[var] ) + ( self.v_ub[var] * self.yd[var] ))
            ...
            dual_obj = (-1*( ( self.biomass_lb * self.d_bio_atp[f'{self.biomass_rxn}_for'] ) + ( self.v_lb['ATPM_for'] * self.d_bio_atp['ATPM_for'] ) )
                        ...
                        + quicksum( self.quu[var]*self.v_ub[var] - self.zul[var]*self.v_lb[var] + ( self.var_o[var]*(1-self.C) + self.v_ub[var]*self.C )*( self.zul[var] - self.qul[var] ) for var in self.v.keys() )
                        + quicksum( self.zdu[var]*self.v_ub[var] - self.qdl[var]*self.v_lb[var] + ( self.var_o[var]*(1-self.C) + self.v_lb[var]*self.C )*( self.qdu[var] - self.zdu[var] ) for var in self.v.keys() )
                       )
